import { useContext, useState, useEffect } from "react";
import {
  Modal,
  ModalOverlay,
  ModalContent,
  ModalHeader,
  ModalCloseButton,
  ModalBody,
  ModalFooter,
  FormControl,
  FormLabel,
  Input,
  Button,
  Stack,
  Text,
  useToast,
} from "@chakra-ui/react";
import { RequestsContexts } from "../../contexts/requestsContexts";
import { ModalsContexts } from "../../contexts/modalsContexts";

const ModalCompanyUpdate = ({ isOpen, onClose }) => {
  const { updateCompany, companies } = useContext(RequestsContexts);
  const { selectedCompany } = useContext(ModalsContexts);

  const initialFormData = {
    name: "",
    adress: "",
    number: "",
    district: "",
    city: "",
    state: "",
  };

  const [formData, setFormData] = useState(initialFormData);
  const [error, setError] = useState("");
  const toast = useToast();

  useEffect(() => {
    if (selectedCompany) {
      setFormData(selectedCompany);
    }
  }, [selectedCompany]);

  const handleInputChange = (e) => {
    setFormData((prevData) => ({
      ...prevData,
      [e.target.name]: e.target.value,
    }));
  };

  const isFormDataUnchanged = () => {
    return Object.entries(formData).every(
      ([key, value]) => selectedCompany[key] === value
    );
  };

  const isAnyDataSubmitted = () => {
    return Object.entries(formData).some(
      ([key, value]) => key !== "id" && value !== ""
    );
  };

  const handleSave = () => {
    if (isFormDataUnchanged()) {
      setError("Nenhum campo foi modificado.");
    } else if (!isAnyDataSubmitted()) {
      setError("Preencha pelo menos um dos campos.");
    } else {
      const duplicateCompany = companies.find(
        (company) =>
          company.name === formData.name && company.id !== selectedCompany.id
      );

      if (duplicateCompany) {
        setError(`Já existe uma empresa com o nome ${formData.name}.`);
      } else {
        setFormData(initialFormData);
        setError("");
        onClose();
        updateCompany(formData);
        toast({
          title: `Empresa ${formData.name} atualizada com sucesso!`,
          status: "success",
          duration: 5000,
          isClosable: true,
        });
      }
    }
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Atualizar Dados</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Stack spacing={4}>
            <FormControl>
              <FormLabel>Nome</FormLabel>
              <Input
                name="name"
                value={formData.name}
                onChange={handleInputChange}
              />
            </FormControl>
            <Stack direction="row">
              <FormControl flex={1}>
                <FormLabel>Endereço</FormLabel>
                <Input
                  name="adress"
                  value={formData.adress}
                  onChange={handleInputChange}
                />
              </FormControl>
              <FormControl flex={0.5}>
                <FormLabel>Número</FormLabel>
                <Input
                  name="number"
                  value={formData.number}
                  onChange={handleInputChange}
                />
              </FormControl>
            </Stack>
            <FormControl>
              <FormLabel>Bairro</FormLabel>
              <Input
                name="district"
                value={formData.district}
                onChange={handleInputChange}
              />
            </FormControl>
            <Stack direction="row">
              <FormControl>
                <FormLabel>Cidade</FormLabel>
                <Input
                  name="city"
                  value={formData.city}
                  onChange={handleInputChange}
                />
              </FormControl>
              <FormControl>
                <FormLabel>Estado</FormLabel>
                <Input
                  name="state"
                  value={formData.state}
                  onChange={handleInputChange}
                />
              </FormControl>
            </Stack>
            {error && <Text color="red.500">{error}</Text>}
          </Stack>
        </ModalBody>
        <ModalFooter>
          <Button colorScheme="blue" onClick={() => handleSave(formData)}>
            Salvar
          </Button>
          <Button variant="outline" onClick={onClose}>
            Cancelar
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  );
};

export default ModalCompanyUpdate;
